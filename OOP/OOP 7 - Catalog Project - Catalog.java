...
public class Catalog {
	...
	public StringBuilder getWholeCatalog() {
		...
		for(int i =0; i< items.size(); i++){
		  newStr.append(items.get(i));
		  newStr.append("-");
		  newStr.append(prices.get(i));
		  newStr.append("-");
		  newStr.append(monthlyPayments.get(i));
		 newStr.append("\n");
	...
	public String getItemDetails(String item) {
		...
		if(items.contains(item)){
		  ...
		  result = item+"-"+prices.get(ind)+"-"+monthlyPayments.get(ind);
		}		
		return result;
	}
	...
	public ArrayList<String> getItemsLessThanAMonthlyPrice(double price) {
		...
		loadWholeCatalog();
		//TODO
			ArrayList<String> lessThanPrice = new ArrayList<>();
			for(int i =0; i<monthlyPayments.size();i++){
			  if(monthlyPayments.get(i) <= price) {
			  lessThanPrice.add(items.get(i)+"-"+prices.get(i)+"-"+monthlyPayments.get(i));
			  }
			}
	...
	public void deleteItemFromCatalog(String item) {
		...
		loadWholeCatalog();
		//TODO
		int del = items.indexOf(item);
		items.remove(del);
		prices.remove(del);
		monthlyPayments.remove(del);
