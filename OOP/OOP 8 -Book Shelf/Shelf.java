...
public class Shelf {
    ...
    public static boolean addBook(int id, String author, String title, int pages ){
      if((isIDUnique(id)==false) || (title.equals("unknown")) ||  (title.equals("")) ||pages <= 0) {
    	  return false;
      }else {
    ...
    public static boolean addBook(Book book ){
    	 if((isIDUnique(book.getId())==false) ||  (book.getTitle().equals("unknown")) || book.getPages() <= 0) {
       	  return false;
         }else {
       	  theBooks.add(new Book(book.getId(),book.getAuthor(),book.getTitle(),book.getPages()));
       	  return true;
         }
