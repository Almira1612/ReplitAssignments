...
public class Book {
    ...
    public static boolean isTitleWord(String word) {
		String[] notWord = new String[]{ "a" , "an", "the","and", "but", "for", "nor", "or", "so", "yet",
				 "at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};
    	for(int i = 0; i < notWord.length; i++) {
    		if(word.toLowerCase().equals(notWord[i])) {
    			return false;
    		}
    	}
    	return true;
    ...
    public void setAuthor(String author) {
        if(author.isEmpty()) {
           ...
           String newStr = "";
           for(String each : spl) {
        	  newStr+=capitalize(each)+" ";       	 
           }
           this.author = newStr.trim();
        }else this.author = capitalize(author);      
    ...
    public void setTitle(String title) {
    	 ...
    	 if(title.isEmpty()) {
            ...
            String newStr = capitalize(spl[0])+" ";
            for(int i = 1; i< spl.length; i++) {
            	if(isTitleWord(spl[i])==false) {
            		newStr+=spl[i].toLowerCase()+" ";
            	}else
         	  newStr+=capitalize(spl[i])+" ";       	 
            }
            this.title = newStr.trim();
         }else this.title = capitalize(title);  
